...
const Callout = ({ type, children }) => {
  const s = {
    ...
    week4:      { bg: "#E6F1FB", border: "#B5D4F4", text: "#0C447C", label: "Coming in Week 4" },
...
function Overview() {
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 500, margin: "0 0 12px", color: "var(--color-text-primary)" }}>Unit 3: Lists, scrolling and the detail screen pattern</h1>
      <P>Almost every real app has a list — a feed, a catalogue, an inbox, a search results page. This week you learn how to build them properly. By the end of Week 3 you will have a scrollable, searchable list that navigates to a detail screen when a row is tapped. That pattern is the backbone of most apps you use every day.</P>
      ...
      <Callout type="warning">Assignment 2 is due before this week starts. Submit it before coming to Session 1.</Callout>
...
function Session1Lab() {
  return (
    <div>
      ...
      <P>You are going to build the main screen of a music album browser — a scrollable list of albums, each displayed as a custom row card. No search or navigation yet — that is Session 2. Budget about 50-60 minutes.</P>
      <H2>Goals</H2>
      <UL items={[
      ...
      <H3>Step 0: Set up your project (~3 min)</H3>
      <Checkbox>Create a new project called AlbumBrowser</Checkbox>
      <Callout type="tip">Add the Navigation Compose dependency to your build.gradle now — you will need it in Session 2: <Code>implementation("androidx.navigation:navigation-compose:2.7.0")</Code></Callout>

      <H3>Step 1: Create the Album data model (~8 min)</H3>
      <P>Define what an album looks like as a data structure, then create a hardcoded list of sample albums.</P>
      <P>For Android — create <Code>Album.kt</Code>:</P>
      <CodeBlock>{`data class Album(
    val id: Int,
    val title: String,
    val artist: String,
    val year: Int,
...
val sampleAlbums = listOf(
    ...
    Album(8, "Abbey Road", "The Beatles", 1969, "Rock", 17, 4.9)
      ...
      <P>For iOS — create <Code>Album.swift</Code>:</P>
      <CodeBlock>{`struct Album: Identifiable {
    let id: Int
    let title: String
    let artist: String
    let year: Int
...
let sampleAlbums: [Album] = [
    ...
    Album(id: 8, title: "Abbey Road", artist: "The Beatles", year: 1969, genre: "Rock", tracks: 17, rating: 4.9)
      ...
      <Callout type="checkpoint">Checkpoint 1: The data model compiles with no errors. Note that the iOS version uses <Code>Identifiable</Code> — SwiftUI needs this to know how to uniquely identify each row in the list.</Callout>
      <Hint title="Why does the iOS struct need Identifiable?">SwiftUI's List needs a way to uniquely identify each item so it can efficiently update only the rows that changed. The Identifiable protocol requires an id field. Compose's LazyColumn uses the key parameter for the same purpose.</Hint>

      <H3>Step 2: Build a basic list — no custom rows yet (~8 min)</H3>
      <P>Before styling the rows, get a plain list rendering first. Confirm the data is showing up before adding complexity.</P>
...
fun AlbumListScreen() {
    LazyColumn(
      ...
      <P>For iOS (SwiftUI):</P>
      <CodeBlock>{`struct AlbumListScreen: View {
    var body: some View {
      ...
      <Callout type="checkpoint">Checkpoint 2: Run the app. You should see a plain scrollable list of album titles. It is not pretty yet — we fix that in the next step.</Callout>
      <Hint title="LazyColumn vs Column — what is the difference?">A regular Column renders ALL its children at once, even if they are off screen. LazyColumn only renders the items currently visible on screen — as you scroll, it creates new rows and recycles old ones. For short lists it does not matter. For a list of 1000 items, Column would crash. Always use LazyColumn/List for scrollable lists of data.</Hint>

      <H3>Step 3: Build a custom row layout (~15 min)</H3>
      <P>Replace the plain Text with a proper album row — artist initial avatar, title, artist name, year, genre badge, and star rating.</P>
      <P>For Android (Compose):</P>
      <CodeBlock>{`@Composable
...
fun AlbumListScreen() {
    LazyColumn(
      ...
      <P>For iOS (SwiftUI):</P>
      <CodeBlock>{`struct AlbumRow: View {
    let album: Album
...
struct AlbumListScreen: View {
    var body: some View {
      ...
      <Callout type="checkpoint">Checkpoint 3: Each album now has a proper row card with an avatar, title, artist, genre badge, year, and star rating. Scroll down — all 8 albums are there.</Callout>

      <H3>Step 4: Ask Claude to translate and compare (~8 min)</H3>
      <Callout type="ai">Paste your full list screen into Claude and use this prompt: "I built an album list screen using [LazyColumn / SwiftUI List]. Please translate it to [SwiftUI List / LazyColumn]. Then explain: what is the equivalent of LazyColumn's items() in SwiftUI, and what does Identifiable do in SwiftUI that Compose handles differently?"</Callout>
      ...
      <CodeBlock>{`// Lab 5 Reflection (Week 3, Session 1)
...
// 3. What was the trickiest part of building the custom row layout?`}</CodeBlock>
      <Callout type="checkpoint">Final checkpoint: Show a TA your styled scrollable album list and walk them through your reflection.</Callout>
      <Callout type="week4">Right now the albums are hardcoded in a Kotlin list or Swift array. In Week 4 you will replace this with a real API call — fetching albums from the internet. The list screen will not need to change much — just the data source.</Callout>
...
function Session2Lab() {
  ...
  return (
    <div>
      ...
      <P>In this lab you will add a search bar that filters the list in real time, handle the empty state when no results are found, and wire up row taps to navigate to an album detail screen. Budget about 50-60 minutes.</P>
      ...
      <H3>Step 0: Open AlbumBrowser from Session 1 (~2 min)</H3>
      <P>Make sure your styled album list from Session 1 is still working before adding anything new.</P>
      <Callout type="tip">If your Session 1 lab is not working, pair with someone who has it working before continuing. Everything today builds on it.</Callout>

      <H3>Step 1: Add a search bar with real-time filtering (~12 min)</H3>
      <P>Add a state variable to hold the search query, a text field at the top of the screen, and filter the album list based on the query.</P>
      <P>For Android (Compose) — update AlbumListScreen:</P>
      <CodeBlock>{`@Composable
fun AlbumListScreen(onAlbumClicked: (Album) -> Unit = {}) {
    ...
    Column(
      ...
      <P>For iOS (SwiftUI) — update AlbumListScreen:</P>
      <CodeBlock>{`struct AlbumListScreen: View {
    ...
    var body: some View {
      ...
      <Callout type="checkpoint">Checkpoint 1: Type in the search bar — the list filters in real time. Clear it — all albums come back. Notice that query is a state variable: when it changes, the UI re-renders with the filtered list automatically.</Callout>

      <H3>Step 2: Handle the empty state (~8 min)</H3>
      <P>When the search returns no results, show a friendly empty state instead of a blank screen.</P>
      <CodeBlock>{`// In AlbumListScreen Column, replace the LazyColumn with:
if (filteredAlbums.isEmpty()) {
    ...
    LazyColumn(
      ...
      <P>For iOS — add the empty state inside the VStack:</P>
      <CodeBlock>{`if filteredAlbums.isEmpty {
    VStack(spacing: 12) {
    ...
    List(filteredAlbums) { album in
      ...
      <Callout type="checkpoint">Checkpoint 2: Search for something that does not exist — like "zzz". You should see the empty state with the music note and message instead of a blank list.</Callout>

      <H3>Step 3: Build the detail screen (~10 min)</H3>
      <P>Build an album detail screen that receives an Album and displays it fully.</P>
      <P>For Android (Compose):</P>
      <CodeBlock>{`@Composable
fun AlbumDetailScreen(album: Album) {
    Column(
      ...
      <P>For iOS (SwiftUI):</P>
      <CodeBlock>{`struct AlbumDetailScreen: View {
    let album: Album
    ...
    var body: some View {
      ...
      <Callout type="checkpoint">Checkpoint 3: The detail screen compiles. You cannot reach it yet — we wire it up in the next step.</Callout>

      <H3>Step 4: Wire up navigation from list to detail (~8 min)</H3>
      <P>Add navigation so tapping a row in the list pushes the detail screen. Use what you learned in Week 2 — this should feel familiar.</P>
      <P>For Android — update MainActivity and add a nav route:</P>
      <CodeBlock>{`// In NavHost — add a detail route:
...
fun AlbumRow(album: Album, onClick: () -> Unit = {}) {
    Row(
      ...
      <P>For iOS — wrap rows in NavigationLink inside AlbumListScreen:</P>
      <CodeBlock>{`List(filteredAlbums) { album in
...
struct ContentView: View {
    var body: some View {
      ...
      <Callout type="checkpoint">Checkpoint 4: Tap any album row — you navigate to the detail screen showing that album's info. Tap back — you return to the list with your search query preserved.</Callout>
      <Hint title="My search query resets when I come back from detail">This is a common issue. The query state variable lives inside AlbumListScreen — it should survive navigation back because the screen is still on the back stack. If it is resetting, check that you are using popBackStack (or the system back button) and not navigating to a fresh home screen.</Hint>

      <H3>Step 5: Ask Claude about combining lists and navigation (~5 min)</H3>
      <Callout type="ai">Ask Claude: "I just combined a searchable list with navigation in [Compose / SwiftUI]. Can you translate the full AlbumListScreen and AlbumDetailScreen to [SwiftUI / Compose]? Then explain: how does tapping a row and passing data to a detail screen differ between the two platforms?"</Callout>
      <Checkbox>Both platform versions have working search, empty state, and detail navigation</Checkbox>

      <H3>Step 6: Reflect (~5 min)</H3>
      <CodeBlock>{`// Lab 6 Reflection (Week 3, Session 2)
...
//    Was there anything you had to look up or ask Claude about?`}</CodeBlock>
      <Callout type="checkpoint">Final checkpoint: Show a TA the full flow — search filtering, empty state, tapping a row to detail, and back. Walk them through your reflection.</Callout>
      <Callout type="week4">Right now albums are hardcoded. In Week 4 you will replace sampleAlbums with a real API call. The list screen, search, and detail screen will not need to change — only where the data comes from.</Callout>
...
function Project() {
  return (
    <div>
      ...
      <Callout type="warning">Submit this assignment by the end of Week 4 Session 1 using the Submit button on this page.</Callout>
      <P>You are going to build a browse app — a scrollable, searchable list that navigates to a detail screen when a row is tapped. The structure is the same as the album browser you built in lab. The theme and content are entirely yours.</P>
      <P>You may build in either Android (Jetpack Compose) or iOS (SwiftUI). You do not need to submit both.</P>

      <H2>Step 1 — pick your theme</H2>
      <P>Choose a category of things you genuinely know about. You need to write at least 8 items, each with at least 4 meaningful fields. The more specific your theme, the more interesting the app.</P>
      ...
      <Callout type="tip">You are not limited to these examples. Any theme works as long as you can write 8 genuine items with real data. Do not make things up — use actual values.</Callout>

      <H2>Step 2 — define your data model</H2>
      <P>Before writing any UI, define your data model. It must have:</P>
      <Checkbox>An <Code>id</Code> field (Int) — required for LazyColumn keys and SwiftUI Identifiable</Checkbox>
      ...
      <P>Example for a video games theme:</P>
      <CodeBlock>{`data class Game(
    val id: Int,
    val title: String,
    val studio: String,
    val year: Int,
    ...
    val rating: Double
      ...
      <P>Each feature below has a clear acceptance criterion — what a reviewer will check when grading.</P>
      ...
      <Callout type="info">The GIF is required. A reviewer cannot grade an app they cannot see running. If your GIF is missing, your submission will be returned ungraded.</Callout>

      <H2>Hints</H2>
      <Hint title="My filter is not updating in real time">Make sure query is a state variable — remember + mutableStateOf in Compose, or @State in SwiftUI. A regular var does not trigger re-renders, so the list will appear frozen even though the value is changing.</Hint>
      <Hint title="My detail screen shows the wrong item">In Compose, make sure you are finding the item by ID from your sample list after navigating — not trying to pass the whole object through the route string. Route strings only carry primitive values like Int or String.</Hint>
      <Hint title="The search query resets when I navigate back from detail">This happens when you navigate() to a new list screen instead of using the system back button or popBackStack(). The system back button pops the detail screen and returns to the existing list screen — with its state intact. Navigate() would create a fresh list screen with an empty query.</Hint>
      <Hint title="My row layout looks cramped or misaligned">Check that your content Column inside the Row uses Modifier.weight(1f) in Compose, or Spacer() in SwiftUI, to fill remaining space. Without it, the trailing element (rating, badge, etc.) will overlap the content column.</Hint>
    </div>
  );
}
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", padding: "1rem 0" }}>
      <div style={{ marginBottom: 4 }}>
        ...
        <p style={{ fontSize: 18, fontWeight: 500, color: "var(--color-text-primary)", margin: 0 }}>Unit 3</p>
      </div>
