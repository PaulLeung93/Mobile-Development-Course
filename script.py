...
with open('src/curriculum/Week 3/week3_unit.tsx', 'r', encoding='utf-8') as f:
    text = f.read()
...
text = re.sub(r'<Callout type="checkpoint">Checkpoint (\d+|Final): (.*?)</Callout>', r'<Checkpoint num="\1">\2</Checkpoint>', text)
text = re.sub(r'<Callout type="checkpoint\">(.*?)</Callout>', r'<Checkpoint num="?">\1</Checkpoint>', text)

text = re.sub(r'<Callout type="info\">(.*?)</Callout>', r'<Note>\1</Note>', text)
text = re.sub(r'<Callout type="tip\">(.*?)</Callout>', r'<Tip>\1</Tip>', text)
text = re.sub(r'<Callout type="warning\">(.*?)</Callout>', r'<Warn>\1</Warn>', text)
text = re.sub(r'<Callout type="ai\">(.*?)</Callout>', r'<AiOpp>\1</AiOpp>', text)
...
# Deal with Hints
text = re.sub(r'<Hint title="(.*?)">(.*?)</Hint>', r'<Tip><b>\1</b> \2</Tip>', text)
text = re.sub(r'<H3>(.*?)</H3>', r'<h4 style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)", margin: "0 0 8px" }}>\1</h4>', text)

with open('src/curriculum/Week 3/week3_unit.tsx', 'w', encoding='utf-8') as f:
    f.write(text)
